...
public class XMLElementFactory {
	...
	private final XMLBroker broker;
		...
		if ((element != null) && (attributes != null)) {
			if (!attributes.isEmpty()) {
				...
				while (entries.hasNext()) {
					...
					Text value = broker.createTextNode(entry.getValue());
	...
	public void elementToAttributes(Element element, HashMap<String, String> attributes) {
		if ((element != null) && (attributes != null)) {
			Element main = findFirstElement(element, ATTRIBUTES);
			if (main != null) {
	...
	protected void updateObjectAttribute(Element elementObject, String attrName, String attrValue) {
		...
		if (attribute != null) {
			...
			if (value != null) {
				...
				value = broker.createTextNode(attrValue);
				attribute.appendChild(value);
			}
		} else {
			attribute = broker.createElement(attrName);
			Text value = broker.createTextNode(attrValue);
			attribute.appendChild(value);
